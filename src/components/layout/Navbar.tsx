...
export function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 bg-cream/85 backdrop-blur-md shadow-[0_4px_30px_-15px_rgba(34,23,16,0.3)]">
      <nav className={`${containerClasses} flex items-center justify-between py-3`}>
        <Link to="/" className="flex items-center gap-2" onClick={() => setOpen(false)}>
          <Logo variant="plain" className="h-10 w-auto sm:h-12" />
        </Link>
        ...
        <div className="hidden items-center gap-8 md:flex">
          {navLinks.map((link) => (
            <button
              ...
              className={`font-display text-sm font-semibold uppercase tracking-wide transition-colors hover:text-orange ${
            ...
            <div className="flex flex-col gap-1 px-6 pb-6 pt-2">
              {navLinks.map((link) => (
                <button
                  key={link.href}
                  onClick={() => handleNavClick(link.href)}
                  className={`rounded-xl px-4 py-3 text-left font-display text-base font-semibold transition-colors hover:bg-orange/10 hover:text-orange ${
